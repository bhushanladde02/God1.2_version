...
public class MainScreen1 extends Activity implements View.OnClickListener,
	...
	int flag=0;
	String tabValue="";
	//int onlyOnceGaaliExport=0;
	//int onlyOnceGaaliExport1=0;
		...
		//	secondTabFlag=0;
	      ...
	      values = getAllGaaliValuesArray(gaaliList);
	        if(values.size() > 0){
	        	adapter.clear();	  
	        	adapter.clear();	
	        	adapter.clear();	adapter.clear();	adapter.clear();	adapter.clear();	adapter.clear();	
...
//			adapter.notifyDataSetChanged();
	...
	private final class AsyncSender extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			pd = new ProgressDialog(MainScreen1.this);
...
//			System.out.println("First Tab");
		...
		//	secondTabFlag=0;
	        ...
	        gaaliList=new ArrayList<Gaali>();//bhushan
			//if(onlyOnceGaaliExport==0){
				//bhushan
			gaaliList.clear();
	        gaaliList= gaaliDAO.getAllGaali();
	        gaaliDAO.deleteGaaliAll();//bhushan  -----------------------------------Removing all the content
	        values.clear();//bhushan
	        values = getAllGaaliValuesArray(gaaliList);
	        if(values.size() > 0){
	        	adapter.clear();
				for (int i = 0; i < values.size(); i++) {
	       ...
	       // onlyOnceGaaliExport=1;
	        //bhushan
	     //  }
		 // else{gaaliList.clear();values.clear();adapter.clear();}
...
//		return gaaliValues.toArray(new String[gaaliValues.size()]);
	...
	public void onConnectDone(final ConnectEvent event) {
		...
		if(event.getResult() == WarpResponseResultCode.SUCCESS){
        	...
        	runOnUiThread(new Runnable() {
				@Override
				public void run() {
					progressDialog = ProgressDialog.show(MainScreen1.this, "", "Recoverable connection error. Recovering session after 5 seconds");
				}
			});
	...
	private void showToastOnUIThread(final String message){
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(MainScreen1.this, message, Toast.LENGTH_LONG).show();
			}
		});
	}
